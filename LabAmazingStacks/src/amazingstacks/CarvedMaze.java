...
public class CarvedMaze extends Maze {
    ...
    public void carve(Cell from, Cell dest) {
        ...
        if (from != null) {
            // Break down the wall between from and dest
    ...
    private void carveMazeUsingStack(Cell start, Cell end) {
        ...
        // Push the start cell onto the stack and carve it
        stack.push(currentCell);
        carve(null, currentCell); // No wall to break at the start
        
        while (!stack.isEmpty()) {
            currentCell = stack.peek();  // Peek at the top of the stack
            
            // Find a random uncarved neighbor
            Cell neighbor = randomUncarvedNeighbor(currentCell);
            
            if (neighbor != null) {
                // Carve the path between the current cell and the neighbor
                carve(currentCell, neighbor);
                
                // Push the neighbor onto the stack to continue from it
                stack.push(neighbor);
            } else {
                // No uncarved neighbors, so backtrack by popping the stack
                stack.pop();
            }
            
            // Stop if we reach the end cell (optional, can carve the full maze if desired)
